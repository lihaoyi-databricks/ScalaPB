for(p <- os.walk(os.pwd)) p.ext match{
  case "scala" | "java" =>
    os.write.over(
      p,
      os.read(p).replace("com.google.protobuf", "grpc_shaded.com.google.protobuf")
    )
...
//    val prefix = if(google) "" else "package grpc_shaded\n"
...
//      p,
//      os.read(p)
//        .replace("\"\"\"package scalapb", "\"\"\"package grpc_shaded.scalapb")
//        .replace("|package scalapb", "|package grpc_shaded.scalapb")
//    )
//  case _ =>
//}